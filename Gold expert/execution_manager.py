...
class ExecutionManager:
    ...
    def optimize_entry(self, signal, market_data):
        """
        Find optimal entry price within time window
        ...
        try:
            ...
            if spread > self.config.TIGHT_SPREAD_THRESHOLD:
                return signal_price, False, f"Spread too wide ({spread*100:.2f}%)"
            ...
            return signal['entry_price'], True, f"Optimization failed, using signal price: {str(e)}"
    ...
    def calculate_position_sizes(self, total_capital, risk_per_trade):
        """
        Calculate position sizes for scaled entry
        Returns: dict with tranche sizes
        """
        total_risk_amount = total_capital * risk_per_trade
        
        return {
            'immediate': total_risk_amount * self.config.ENTRY_TRANCHES['immediate'],
            'pullback': total_risk_amount * self.config.ENTRY_TRANCHES['pullback'],
            'confirmation': total_risk_amount * self.config.ENTRY_TRANCHES['confirmation']
        }
    ...
    def manage_stop_loss(self, position, market_data, targets_hit):
        """
        Dynamically manage stop loss based on targets hit
        Returns: (new_stop_loss, reason)
        """
            ...
            if atr:
                ...
                if direction == 'BUY':
                    ...
                    if new_sl > current_sl:  # Only move up
                        return new_sl, f"Trailing SL (ATR: ${atr:,.0f})"
                else:  # SELL
                    new_sl = current_price + trail_distance
    ...
    def create_execution_plan(self, signal, market_data):
        """
        Create complete execution plan with all enhancements
        ...
        position_sizes = self.calculate_position_sizes(500, 0.01)  # Default values
