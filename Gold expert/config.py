...
IMPORTANT: This bot fetches data from Binance (PAXGUSDT) but generates signals
for TradingView CFD XAUUSD trading.

Price Differences:
- Binance PAXGUSDT: ~$2,045 per token (1 token = 1 troy oz gold)
- TradingView XAUUSD CFD: Price varies by broker format
- Actual spot gold: ~$2,650-2,700 per troy ounce

The SIGNALS (BUY/SELL direction, stop loss %, take profit %) are valid
regardless of the price format your broker uses.
"""
...
SYMBOL = "PAXGUSDT"  # Data source: Binance Paxos Gold
TRADING_SYMBOL = "XAUUSD"  # Your TradingView CFD symbol
