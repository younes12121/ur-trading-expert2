...
class NewsFetcher:
    """
    Fetches Gold news from Kitco RSS feed
    100% FREE - No API key needed!
    """
    
    def __init__(self):
        # Kitco Gold News RSS feed
        self.rss_url = "https://www.kitco.com/rss/latest/gold.xml"
        
    def get_crypto_news(self, limit: int = 10) -> Optional[List[Dict]]:
        """
        Get latest Gold news from Kitco RSS feed
        ...
        try:
            ...
            for item in root.findall('.//item')[:limit]:
                ...
                # Only include Gold-related news
                title = title_elem.text if title_elem is not None else ''
                if any(keyword in title.lower() for keyword in ['gold', 'xau', 'fed', 'inflation', 'dollar']):
                    news_items.append({
                        'title': title,
    ...
    def check_high_impact_news(self, hours_back: int = 2) -> Dict:
        ...
        try:
            ...
            # Consider it high impact if there are 2+ Gold news items in the timeframe
            has_high_impact = len(recent_news) >= 2
            
            if has_high_impact:
                warning = f"⚠️ {len(recent_news)} Gold news item(s) in last {hours_back} hours!"
            else:
                warning = None
    ...
    def get_news_summary(self) -> str:
        ...
        if not news_check['recent_news']:
            return "✅ No major Gold news in the last 4 hours - Safe to trade"
        
        summary = f"\n📰 IMPORTANT GOLD NEWS (Last 4 hours):\n"
        summary += "=" * 80 + "\n"
...
class NewsAPIFetcher:
    ...
    def get_btc_news(self, hours_back: int = 4) -> Optional[List[Dict]]:
        ...
        try:
            ...
            params = {
                'q': 'Gold OR XAU OR Federal Reserve',
                'from': from_date,
                'sortBy': 'publishedAt',
                'language': 'en',
                'apiKey': self.api_key
