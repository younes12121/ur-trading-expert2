...
class UltraAPlusFilter(EnhancedAPlusFilter):
    ...
    def check_dxy_strength(self, direction):
        """
        Enhancement 16: DXY Strength (USD Index)
        Inverse correlation with Gold
        Impact: +5-8% win rate
        """
        try:
            dxy_data = self.advanced_fetcher.get_dxy_strength()
            
            if not dxy_data:
                return True, "DXY data unavailable (assuming OK)"
            
            dxy_val = dxy_data['dxy_value']
            
            # Gold usually moves inverse to DXY
            
            if direction == 'BUY':
                # For Gold longs, we want Weak DXY
                if dxy_val < 100:
                    return True, f"Weak DXY ({dxy_val}) - bullish for Gold"
                elif dxy_val < 103:
                    return True, f"Neutral/Weak DXY ({dxy_val})"
                else:
                    return False, f"Strong DXY ({dxy_val}) - bearish for Gold"
            
            elif direction == 'SELL':
                # For Gold shorts, we want Strong DXY
                if dxy_val > 105:
                    return True, f"Strong DXY ({dxy_val}) - bearish for Gold"
                elif dxy_val > 103:
                    return True, f"Neutral/Strong DXY ({dxy_val})"
                else:
                    return False, f"Weak DXY ({dxy_val}) - bullish for Gold"
            
            return False, "Direction not BUY or SELL"
            
        except Exception as e:
            return True, f"DXY check failed (assuming OK): {str(e)}"
    ...
    def check_social_sentiment(self, direction):
        ...
        """
        try:
            sentiment_data = self.advanced_fetcher.get_gold_sentiment()
    ...
    def filter_signal_ultra(self, signal_data, market_data):
        ...
        # 16. DXY Strength
        dxy_ok, dxy_msg = self.check_dxy_strength(signal_data['direction'])
        reasons['dxy_strength'] = f"{'[OK]' if dxy_ok else '[FAIL]'} {dxy_msg}"
        if not dxy_ok:
            all_checks_passed = False
...
if __name__ == "__main__":
    ...
    if market_data:
        ...
        # Show advanced 3
        print("\nADVANCED 3 CRITERIA:")
        advanced = ['funding_rate', 'dxy_strength', 'social_sentiment']
        for c in advanced:
            if c in reasons:
                print(f"   {reasons[c]}")
